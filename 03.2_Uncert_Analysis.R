...
load("/projectnb/dietzelab/tmccabe/mccabete/Fire_forecast_509/output/mcmc/20190507.200s_1.small_IC.convergence_passed_GBR_test.JAGS_run_.Rdata")
...
legend("topright",legend=c("IC,Pra,Pro"),col="green",lty=1,lwd=3, cex=0.75)
plot.run(xlim = c(101,106),ylim = c(min(N.IPE.ci),max(N.IPE.ci)))
...
legend("topright",legend=c("IC,Pra"),col="red",lty=1,lwd=3, cex=0.75)
plot.run(xlim = c(101,106),ylim = c(min(N.IPE.ci),max(N.IPE.ci)))
...
legend("topright",legend=c("IC"),col="yellow",lty=1,lwd=3, cex=0.75)
plot.run(xlim = range(time),ylim = c(min(ci),max(ci)))
lines(((tail(time1,1)+8):(tail(time1,1)+8+time2))[-1],N.det,col="purple",lwd=3)
legend("topright",legend=c("Obs","Fit","Pre"),col=c(1,"blue","purple"),
       lty=c(NA,1,1),pch=c(1,NA,NA),lwd=c(NA,1,1), cex=0.75)
...
#ecoforecastR::ciEnvelope(((tail(time1,1)+8):(tail(time1,1)+8+time2))[-1],V.pred.rel[2,],V.pred.rel[3,],col="red")
legend(103, 1,legend=c("IC","IC,Par","IC,Pra,Pro"),col=c("yellow","green","red"),
